...
CPPFLAGS_QTMULTIMEDIA ?= $(shell PKG_CONFIG_PATH=$(PKG_CONFIG_PATH) $(PKGCONFIG) Qt5Multimedia --cflags $(STDERR_TO_DEVNULL)) -DQT_NO_KEYWORDS
...
CPPFLAGS          += $(CPPFLAGS_QTCORE) $(CPPFLAGS_QTGUI) $(CPPFLAGS_QTWIDGETS) $(CPPFLAGS_QTSVG) $(CPPFLAGS_QTMULTIMEDIA) $(CPPFLAGS_QTMULTIMEDIAWIDGETS) $(CPPFLAGS_QTNETWORK) $(CPPFLAGS_XML) $(CPPFLAGS_GLIB) $(CPPFLAGS_PNG) $(CPPFLAGS_JPEG) -Iinclude -Ilibs
ASSIMP_INTERNAL    ?= no
...
$(INSTALLDIR)/radiant.$(EXE): LIBS_EXTRA := $(LIBS_GL) $(LIBS_DL) $(LIBS_XML) $(LIBS_GLIB) $(LIBS_QTWIDGETS) $(LIBS_QTSVG) $(LIBS_QTMULTIMEDIA) $(LIBS_QTMULTIMEDIAWIDGETS) $(LIBS_QTNETWORK) $(LIBS_ZLIB)
